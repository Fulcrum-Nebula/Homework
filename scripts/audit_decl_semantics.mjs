...
const uses=[],errors=[],special=[],diags={},diagnostics=[];
...
for(const e of data.entries){
 ...
 try {const tree=parse(snl);const r=resolve(tree,catalog);diagnostics.push({id:e.id,diagnostics:r.diagnostics});for(const d of r.diagnostics)diags[d.code]=(diags[d.code]||0)+1;
 ...
 }catch(err){errors.push({id:e.id,error:String(err),stack:err?.stack??null,original:snl});}
}
...
const result={counts,affectedEntries:new Set(uses.map(u=>u.id)).size,uses,errors,diags,special,strings,diagnostics};
