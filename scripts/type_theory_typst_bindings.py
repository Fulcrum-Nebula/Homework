#!/usr/bin/env python3
"""Apply and validate the opaque Fulcrum Typst declarations for TypeTheory's Macro closure."""
...
def validate_workspace(root: Path | str) -> dict[str, Any]:
    ...
    wanted = set(CLOSURE)
    if len(CLOSURE) != 63 or len(wanted) != 63:
        raise ValidationError("internal closure must contain exactly 63 unique Macro identities")
...
def apply_workspace(root: Path | str) -> int:
    root = Path(root)
    index = _macro_index(root)
    missing = sorted(set(CLOSURE) - set(index))
    if missing:
        raise ValidationError(f"missing closure Macros: {missing}")
    changed = 0
    for package, name in CLOSURE:
        path, envelope = index[(package, name)]
        macro = _require_dict(envelope.get("macro"), "macro")
        expected = expected_extension(package, name)
        if macro.get(EXTENSION_KEY) != expected:
            macro[EXTENSION_KEY] = expected
...
def main() -> int:
    ...
    parser.add_argument("--apply", action="store_true", help="write the exact declarations before validating")
    args = parser.parse_args()
    changed = apply_workspace(args.workspace) if args.apply else 0
